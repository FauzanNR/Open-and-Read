...
interface ApiService {
    ...
    @GET("advancedsearch.php?q=openlibrary_work:(*)+AND+(collection:(inlibrary)+OR+(!collection:(printdisabled)))+AND+(lending___available_to_browse:true+OR+lending___available_to_borrow:true)+AND+languageSorter:(%22English%22)+AND+openlibrary_subject:openlibrary_staff_picks&offset=2&output=json")
